...
class SemicharDataset(data.Dataset):
    def __init__(self, root_path, seq_length, predefined=False):
        ...
        if not predefined:
            ...
            self.tokenized_data = self.tokenized_data[
                                  :len(self.tokenized_data) - len(self.tokenized_data) % self.seq_length]
            ...
            self.augmentator = nac.KeyboardAug(aug_char_min=0, aug_char_p=0.4, aug_word_p=0.4, aug_word_min=0,
                                               aug_word_max=int(0.7 * self.seq_length), special_char=False,
                                               tokenizer=lambda x: x.split(), reverse_tokenizer=lambda x: x)
    ...
    def tokenize(self, root_path):
        ...
        splitted = [token.text for token in x if
                    not (token.text.isspace() or (token.text[0].isdigit() and token.text[-1].isdigit()) or (
                                token.text[0] in string.punctuation and token.text[-1] in string.punctuation))]
        return splitted
